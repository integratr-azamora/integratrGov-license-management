<template>
   <br>
    <div class="container bor-style-solid-perma container5" >
       <div class="container">
         <div class="bor-bottom row justify-content-center">
             <p class="inter fs-24 fw-bold border-bottom-1">Gotham</p>
          </div>
          <!-- heading1 -->
          <div class="row">
             <div class="col d-flex align-items-center">
                <p class="h1 fw-bold mt-4">Heading 1</p>
             </div>
             <div class="col">
                <div class="row">
                   <p class="font-style-gotham-h mt-4"><b>Font size:</b> 32px/2(em/rem)</p>
                </div>
                <div class="row">
                   <p class="font-style-gotham-h mt-1"><b>Font weight:</b> Bold</p>
                </div>
             </div>
          </div>
          <!-- heading 2 -->
          <div class="row">
             <div class="col d-flex align-items-center">
                <p class="h2 fw-bold mt-4">Heading 2</p>
             </div>
             <div class="col">
                <div class="row">
                   <p class="font-style-gotham-h mt-3"><b>Font size:</b> 24px/1.5(em/rem)</p>
                </div>
                <div class="row">
                   <p class="font-style-gotham-h mt-1"><b>Font weight:</b> Bold</p>
                </div>
             </div>
          </div>
          <!-- heading 3 -->
          <div class="row">
             <div class="col d-flex align-items-center">
                <p class="h3 fw-bold mt-4">Heading 3</p>
             </div>
             <div class="col">
                <div class="row">
                   <p class="font-style-gotham-h mt-3"><b>Font size:</b> 16px/1(em/rem)</p>
                </div>
                <div class="row">
                   <p class="font-style-gotham-h mt-1"><b>Font weight:</b> Book</p>
                </div>
             </div>
          </div>
          <!--Body/Paragraph 4-->
          <div class="row">
             <div class="col d-flex align-items-center">
                <p class="font-style-gotham-b1 fs-16 mt-4">Body/Paragraph</p>
             </div>
             <div class="col">
                <div class="row">
                   <p class="font-style-gotham-h mt-3"><b>Font size:</b> 16px/1(em/rem)</p>
                </div>
                <div class="row">
                   <p class="font-style-gotham-h mt-1"><b>Font weight:</b> Book</p>
                </div>
             </div>
          </div>
          <!--Body/Paragraph 5-->
          <div class="row">
             <div class="col d-flex align-items-center">
                <p class="font-style-gotham-b2 fs-14 mt-4">Body/Paragraph</p>
             </div>
             <div class="col">
                <div class="row">
                   <p class="font-style-gotham-h mt-3"><b>Font size:</b> 14px/0.875(em/rem)</p>
                </div>
                <div class="row">
                   <p class="font-style-gotham-h mt-1"><b>Font weight:</b> Book</p>
                </div>
             </div>
          </div>
       </div>
       <!-- Roboto-headings-->
       <div class="container">
          <div class="bor-style-solid-perma row justify-content-center mt-4">
             <div class="col borderStyle ">
                <p class="roboto-headings fw-bold"> Roboto</p>
             </div>
          </div>
          <!-- heading 1-->
          <div class="row">
             <div class="col d-flex align-items-center">
                <p class="heading1 mt-4 Roboto">Heading 1</p>
             </div>
             <div class="col">
                <div class="row">
                   <p class="font-style-roboto-h mt-4"><b> Font size:</b> 32px/2(em/rem)</p>
                </div>
                <div class="row">
                   <p class="font-style-roboto-h mt-2"><b>Font weight:</b> Black</p>
                </div>
             </div>
          </div>
          <!-- heading 2 -->
          <div class="row">
             <div class="col d-flex align-items-center">
                <p class="heading2 fw-bold mt-4">Heading 2</p>
             </div>
             <div class="col">
                <div class="row">
                   <p class="font-style-roboto-h mt-4"><b>Font size:</b> 24px/1.5(em/rem)</p>
                </div>
                <div class="row">
                   <p class="font-style-roboto-h mt-2"><b>Font weight:</b> Bold</p>
                </div>
             </div>
          </div>
          <!-- heading 3 -->
          <div class="row">
             <div class="col d-flex align-items-center">
                <p class="heading3 fw-bold mt-4">Heading 3</p>
             </div>
             <div class="col">
                <div class="row">
                   <p class="font-style-roboto-h  mt-4"><b>Font size:</b> 16px/1(em/rem)</p>
                </div>
                <div class="row">
                   <p class="font-style-roboto-h  mt-2"><b>Font weight:</b> Bold</p>
                </div>
             </div>
          </div>
          <!-- heading 4 -->
          <div class="row">
             <div class="col d-flex align-items-center">
                <p class="heading4 fw-bold mt-4">Heading 4</p>
             </div>
             <div class="col">
                <div class="row">
                   <p class="font-style-roboto-h  mt-4"><b>Font size:</b> 16px/1(em/rem)</p>
                </div>
                <div class="row">
                   <p class="ffont-style-roboto-h  mt-2"><b>Font weight:</b> Meduim</p>
                </div>
             </div>
          </div>
          <!-- heading 5 -->
          <div class="row">
             <div class="col d-flex align-items-center">
                <p class="heading5 fw-bold mt-4">Heading 5</p>
             </div>
             <div class="col">
                <div class="row">
                   <p class="font-style-roboto-h fs-20 mt-4"><b>Font size:</b> 14px/0.875(em/rem)</p>
                </div>
                <div class="row">
                   <p class="font-style-roboto-h fs-20 mt-2"><b>Font weight:</b> Bold</p>
                </div>
             </div>
          </div>
          <!--Body/Paragraph 6-->
          <div class="row">
             <div class="col d-flex align-items-center">
                <p class="font-style-roboto-p1 mt-4"> Body/Paragraph</p>
             </div>
             <div class="col">
                <div class="row">
                   <p class="font-style-roboto-h mt-4"><b>Font size:</b> 14px/0.875(em/rem)</p>
                </div>
                <div class="row">
                   <p class="font-style-roboto-h mt-2"><b>Font weight:</b> Regular</p>
                </div>
             </div>
          </div>
          <!--Body/Paragraph 7-->
          <div class="row">
             <div class="col d-flex align-items-center">
                <p class="font-style-roboto-p2 mt-4">Body/Paragraph</p>
             </div>
             <div class="col">
                <div class="row">
                   <p class="font-style-roboto-h fs-20 mt-4"><b>Font size:</b> 16px/0.875(em/rem)</p>
                </div>
                <div class="row">
                   <p class="font-style-roboto-h fs-20 mt-2"><b>Font weight:</b> Regular</p>
                </div>
             </div>
          </div>
       </div>
       <!-- Poppins    -->
       <div class="container">
          <div class="bor-style-solid-perma row justify-content-center mt-4">
             <div class="col borderStyle">
                <p class="poppins-paragraph fw-bold">Poppins</p>
             </div>
          </div>
          <!--Body/Paragraph -->
          <div class="row">
             <div class="col d-flex align-items-center">
                <p class="paragraph1 mt-4"> Body/Paragraph</p>
             </div>
             <div class="col">
                <div class="row">
                   <p class="poppins-body1 mt-4"><b>Font size:</b> 16px/1(em/rem)</p>
                </div>
                <div class="row">
                   <p class="poppins-body1 fs-20 mt-2"><b>Font weight:</b> Regular</p>
                </div>
             </div>
          </div>
       </div>
    </div>
 </template>